...
export default function App() {
  ...
  if (screen === 'timeline') return wrapAnimated(<TimelineScreen isPro={isPro} canUseAI={true} onBack={goHome} />);
  ...
  if (screen === 'case_intake') return wrapAnimated(<CaseIntakeScreen isPro={isPro} hasCaseIntake={true} onBack={goHome} onUpgrade={() => setScreen('upgrade')} />);
...
const s = StyleSheet.create({
  ...
  aiPowered:  { fontSize: IS_IPAD ? 9 : 7, color: C.gray, letterSpacing: 0.5, marginTop: 2 },
  logo:       { fontSize: IS_IPAD ? 32 : 26, fontWeight: '900', color: C.accent, letterSpacing: IS_IPAD ? 5 : 4 },
  logoSub:    { fontSize: IS_IPAD ? 10 : 8, color: C.gray, letterSpacing: 1.5, marginTop: 2 },
  ...
  disclaimer: { marginHorizontal: IS_IPAD ? 24 : 16, marginBottom: 8, backgroundColor: '#0d1a10', borderLeftWidth: 2, borderLeftColor: C.amber + '80', borderRadius: 6, padding: IS_IPAD ? 8 : 7 },
  disclaimerTxt: { color: C.amber + 'cc', fontSize: IS_IPAD ? 10 : 9 },
  ...
  moduleCard: { backgroundColor: C.card, borderRadius: IS_IPAD ? CARD.radiusLg : CARD.radius, padding: IS_IPAD ? CARD.paddingLg : 20, minHeight: IS_IPAD ? undefined : 118, justifyContent: 'center', borderWidth: CARD.borderWidth, borderColor: C.border, borderTopWidth: 2, borderTopColor: C.accent },
  securityCard: { borderColor: C.accentDim, backgroundColor: '#0a1520' },
  moduleIcon: { fontSize: IS_IPAD ? 28 : 34, marginBottom: IS_IPAD ? 8 : 10 },
  moduleTitle:{ color: C.text, fontWeight: '700', fontSize: IS_IPAD ? 16 : 15, marginBottom: 3 },
  moduleDesc: { color: C.textMid, fontSize: IS_IPAD ? 13 : 12, lineHeight: IS_IPAD ? 18 : 16 },
