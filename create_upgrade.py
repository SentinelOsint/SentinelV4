...
export default function UpgradeScreen({ reason = 'expired', onBack, onSubscribe }: Props) {
  ...
  const [soloPrice,  setSoloPrice]  = useState('$29.99');
    ...
    (async () => {
      ...
      if (ok) {
        ...
        products.forEach((p: any) => {
          if (p.productId === PRODUCT_IDS.SOLO && p.localizedPrice) setSoloPrice(p.localizedPrice);
  ...
  const handlePurchase = async (productId: string, tier: 'solo' | 'pro') => {
    if (!iapReady) {
    ...
    await purchaseSubscription(
      ...
      (purchasedTier) => {
        setLoading(false);
        Alert.alert('✅ Success', `Welcome to Sentinel ${purchasedTier === 'pro' ? 'Pro' : 'Solo'}!`, [
  ...
  const handleRestore = async () => {
    ...
    await restorePurchases(
      (restoredTier) => {
        setRestoring(false);
        Alert.alert('✅ Restored', `Your ${restoredTier === 'pro' ? 'Pro' : 'Solo'} subscription has been restored.`, [
      ...
      <ScrollView contentContainerStyle={s.scroll}>
        ...
        <View style={s.planCard}>
          <View style={s.planHeader}>
            ...
            <Text style={s.planPrice}>{soloPrice}<Text style={s.planPer}>/mo</Text></Text>
          </View>
          <View style={s.featureList}>
            {[
              '✓ All 12 OSINT modules',
              '✓ Unlimited cases',
          ...
          <TouchableOpacity
            style={[s.buyBtn, s.soloBtn]}
            onPress={() => handlePurchase(PRODUCT_IDS.SOLO, 'solo')}
            disabled={loading}
          >
            {loading ? <ActivityIndicator color={C.bg} /> : <Text style={s.buyBtnTxt}>Subscribe to Solo</Text>}
          </TouchableOpacity>
        </View>
        ...
        <View style={[s.planCard, s.proCard]}>
          <View style={s.foundingBadge}>
            <Text style={s.foundingTxt}>⭐ FOUNDING MEMBER — Lock in $79.99/mo forever</Text>
          </View>
          <View style={s.planHeader}>
...
const s = StyleSheet.create({
  ...
  foundingBadge: { backgroundColor: C.accentDim, borderRadius: 8, padding: 8, marginBottom: 12 },
  foundingTxt:   { color: C.accent, fontSize: 12, fontWeight: '700', textAlign: 'center' },
  ...
  soloBtn:       { backgroundColor: C.card, borderWidth: 1, borderColor: C.border },
...
with open('src/screens/UpgradeScreen.tsx', 'w') as f:
    f.write(upgrade_code)
print('UpgradeScreen.tsx updated!')
