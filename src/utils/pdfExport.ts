  ...
  const rows = results.map(r => {
    ...
    if (r.type === 'link') return `
      <div class="result-row">
        ...
        <div class="result-value"><a href="${r.value}" class="result-link">${r.label}</a><br/><span style="font-size:10px;color:#9bb0c4;">${r.value}</span></div>
      </div>`;
    return `
      <div class="result-row">
        <div class="result-label">${r.label}</div>
  ...
  const html = `<!DOCTYPE html><html><head><meta charset="utf-8"/><style>${CSS}</style></head><body>
    <div class="page">
      <div class="header">
        ...
        <div class="logo-sub">OSINT FIELD TOOLKIT · NORTH AMERICA EDITION</div>
        <div class="report-title">${module} Report</div>
        <div class="meta-row">
          <div class="meta-item"><strong>Query:</strong> ${query}</div>
          <div class="meta-item"><strong>Date:</strong> ${new Date().toLocaleString('en-US')}</div>
...
export async function exportCasePDF(caseData: CaseReport): Promise<void> {
  ...
  const html = `<!DOCTYPE html><html><head><meta charset="utf-8"/><style>${CSS}</style></head><body>
    <div class="page">
      <div class="header">
        <div class="logo">SENTINEL</div>
        <div class="logo-sub">OSINT FIELD TOOLKIT · CASE REPORT</div>
...
export async function exportInvestigationReport(t: ReportTemplate): Promise<void> {
  ...
  const html = `<!DOCTYPE html><html><head><meta charset="utf-8"/><style>${CSS}</style></head><body>
    <div class="page">
      <div class="header">
        <div class="logo">SENTINEL</div>
        <div class="logo-sub">OSINT FIELD TOOLKIT · INVESTIGATION REPORT</div>
        <div class="report-title">${caseData.title}</div>
        <div class="meta-row">
          <div class="meta-item"><strong>Case ID:</strong> ${caseData.id}</div>
          <div class="meta-item"><strong>Status:</strong> <span class="badge ${statusClass}">${caseData.status.toUpperCase()}</span></div>
